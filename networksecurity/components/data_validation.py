...
class DataValidation:
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            ...
            if not status:
                error_message="Train dataframe does not contain all clolumns.\n"
            status = self.validate_number_of_columns(dataframe=test_dataframe)
            if not status:
                error_message="Test dataframe does not contain all clolumns.\n"

            # Validate Numerical Columns
            status = self.validate_numerical_columns(dataframe=train_dataframe)
            if not status:
                error_message = "Train dataframe does not contain any numerical columns"
            status = self.validate_numerical_columns(dataframe=test_dataframe)
            if not status:
                error_message = "Test dataframe does not contain any numerical columns"
